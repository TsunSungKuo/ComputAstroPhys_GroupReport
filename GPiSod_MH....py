...
# -----------------------------------------
# # Gaussian Process Functions for WENO
# -----------------------------------------
...
def train_gp_and_predict(X_train, y_train, X_test):
    ...
    # Compute the covariance matrices using optimized hyperparameters
...
# GP-WENO reconstruction
def GPWENO(U, order=3):
    x_train = np.linspace(0, L, N_In).reshape(-1, 1)
    U_gp = np.empty(U.shape)
    for i in range(3):  # for each conserved variable
